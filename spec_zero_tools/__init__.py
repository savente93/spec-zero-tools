...
def update_pyproject_dependencies(dependencies: dict, schedule: SupportSchedule):
    # iterate by idx becasue we want to update it inplace
    for i in range(len(dependencies)):
        dep_str = dependencies[i]
        pkg, spec = parse_pep_dependency(dep_str)
    ...
    for pkg, pkg_data in dep_table.items():
        ...
        if isinstance(pkg_data, str):
            if not is_url_spec(pkg_data):
                ...
                # we don't do antyhing with url spec dependencies
                continue
        else:
...
def update_pyproject_toml(pyproject_data: dict, schedule_data: Sequence[SupportSchedule]):

    now = datetime.datetime.now(datetime.UTC)
    try:
        new_version = cast(SupportSchedule,next(
            filter(
        ...
        raise RuntimeError(
            "Could not find schedule that applies to current time, perhpas your schedule is oudated."
        )
